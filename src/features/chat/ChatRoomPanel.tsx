...
export function ChatRoomPanel({ onClose }: ChatRoomPanelProps) {
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const isModerator = role === 'operator' || role === 'pjc' || role === 'sys_admin' || role === 'noc';
  ...
  const getRoleBadge = (senderId: string) => {
    ...
    const badges: Record<string, JSX.Element> = {
      noc: <span className="bg-red-500 text-white font-extrabold text-[8px] px-1 py-0.2 rounded uppercase shrink-0">NOC</span>,
      sys_admin: <span className="bg-purple-500 text-white font-extrabold text-[8px] px-1 py-0.2 rounded uppercase shrink-0">ADM</span>,
      pjc: <span className="bg-amber-500 text-black font-extrabold text-[8px] px-1 py-0.2 rounded uppercase shrink-0">PJC</span>,
      operator: <span className="bg-sky-500 text-white font-extrabold text-[8px] px-1 py-0.2 rounded uppercase shrink-0">OP</span>,
    };
  ...
  return (
    <div className="w-full h-full flex flex-col bg-[#111216] select-none text-white">
      ...
      {/* Messages List Area */}
      <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-[#13151b] scrollbar-thin">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center h-full text-slate-500 p-6 text-center">
            ...
            <p className="text-[10px] text-slate-600 mt-1">Kirim pesan pertama di saluran ini!</p>
          </div>
        ) : (
          messages.map((msg) => {
            ...
            return (
              ...
              >
                {/* Avatar */}
                <div className="w-7 h-7 rounded-full overflow-hidden border border-slate-700 bg-slate-800 shrink-0">
                  {msg.sender_avatar ? (
                    <img src={msg.sender_avatar} alt="avatar" className="w-full h-full object-cover" />
                  ) : (
                    <div className="w-full h-full bg-slate-700 flex items-center justify-center text-[10px] font-bold uppercase">
                ...
                <div className="flex flex-col space-y-0.5">
                  ...
                  <div
                    className={`rounded-2xl px-3.5 py-2 text-xs break-all leading-normal border shadow-sm ${
                      isMe
                        ...
                        : 'bg-slate-800/80 border-slate-700/60 text-slate-100 rounded-tl-none'
                    }`}
                  >
                    {msg.message}
                  </div>
                  ...
                  {/* Timestamp */}
                  <span className={`text-[8px] text-slate-500 font-semibold ${isMe ? 'text-right' : ''}`}>
      ...
      <div className="p-3 bg-[#111216] border-t border-slate-800 shrink-0">
        {!canChat ? (
          ...
          <form onSubmit={handleSendMessage} className="flex gap-2">
            <input
              ...
              className="flex-1 bg-slate-900 border border-slate-800 focus:border-slate-700 outline-none rounded-xl px-3.5 py-2 text-xs font-semibold placeholder:text-slate-600 text-white disabled:opacity-50"
            />
            <button
              type="submit"
