...
Setiap fitur diberi label **[P0]** (Must Have), **[P1]** (Should Have), **[P2]** (Nice to Have).
...
### 7.2 Schema Database Supabase (Target Produksi)

```sql
-- Tabel channels (menggantikan hardcoded config.ts)
CREATE TABLE channels (
  id          SERIAL PRIMARY KEY,
  number      INTEGER UNIQUE NOT NULL CHECK (number BETWEEN 0 AND 999),
  name        TEXT NOT NULL,
...
-- Tabel user_profiles (data publik pengguna)
CREATE TABLE user_profiles (
  id           UUID PRIMARY KEY REFERENCES auth.users(id) ON DELETE CASCADE,
  display_name TEXT NOT NULL,
  call_sign    TEXT NOT NULL CHECK (call_sign ~ '^[A-Z0-9]{5}$'),
  location     TEXT,
  avatar_url   TEXT,
  updated_at   TIMESTAMPTZ DEFAULT NOW()
);

-- Tabel ptt_sessions (audit log transmisi)
CREATE TABLE ptt_sessions (
  ...
  duration_ms  INTEGER,
  audio_mode   TEXT CHECK (audio_mode IN ('discussion', 'music'))
);
...
CREATE POLICY "Users can insert own sessions" ON ptt_sessions FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Users can read own sessions" ON ptt_sessions FOR SELECT USING (auth.uid() = user_id);
```

### 7.3 Supabase Edge Functions yang Diperlukan

| Function | Trigger | Fungsi |
|---|---|---|
| `validate-channel-access` | Sebelum join channel | Cek apakah user diizinkan masuk channel |
| `ptt-rate-limiter` | Setiap broadcast voice_chunk | Batasi max transmisi per user per detik |
| `cleanup-stale-sessions` | Cron setiap 5 menit | Hapus sesi PTT yang tidak ter-close dengan benar |
...
| Tier | Harga | Fitur |
|---|---|---|
| **Gratis** | Rp 0 | Channel 0–99, Discussion mode, 1 tema (Classic), max 10 user per channel |
| **Premium** | Rp 25.000/bulan | Channel 0–999, Music/Karaoke mode, semua 8 tema, max 50 user per channel, callsign kustom |
| **Komunitas** | Rp 150.000/bulan | Semua Premium + namespace channel private + branding kustom minimal |
| **White-Label** | Negosiasi | Aplikasi terpisah dengan branding penuh klien, server dedicated |

### 9.2 White-Label Pricing

| Paket | Biaya Setup | Biaya Bulanan | Inklusif |
|---|---|---|---|
| Starter | Rp 5 juta | Rp 1 juta | Rebrand app, 5 channel private, 100 user |
| Pro | Rp 15 juta | Rp 3 juta | Rebrand + APK private + 20 channel, 500 user |
| Enterprise | Rp 50 juta | Custom | Full custom + server dedicated + SLA 99.9% |

---

## 10. Metrik Keberhasilan

### 10.1 Metrik Produk (Dipantau Harian)

| Metrik | Definisi | Target 3 Bulan |
|---|---|---|
| DAU | Daily Active Users | 500 |
| Sesi PTT per hari | Total transmisi yang berhasil | 5.000 |
| Rata-rata durasi sesi | Menit per sesi per user | 8 menit |
| Audio success rate | % transmisi yang diterima tanpa error | > 95% |
| Channel 100 loopback usage | Test mode usage (proxy untuk onboarding) | < 20% sesi (artinya user sudah "lulus" onboarding) |

### 10.2 Metrik Teknis (Dipantau Real-time)

| Metrik | Alert Threshold |
|---|---|
| Audio latency P95 | > 500ms |
| WebRTC connection success rate | < 85% |
| Supabase channel subscription errors | > 1% |
| App crash rate | > 0.5% |
| TURN server relay usage | > 40% (indikasi masalah NAT) |

### 10.3 Metrik Bisnis (Dipantau Bulanan)

| Metrik | Target 6 Bulan |
|---|---|
| MAU (Monthly Active Users) | 5.000 |
| Premium conversion rate | 5% dari MAU |
| MRR (Monthly Recurring Revenue) | Rp 15 juta |
| NPS (Net Promoter Score) | ≥ 40 |
| White-label clients signed | 2 kontrak |

---

## 11. Roadmap Rilis

### Fase 0 — Perbaikan Kritis (Minggu 1–2)
*Sebelum distribusi ke siapapun*

- [ ] Rotasi semua credential yang bocor
- [ ] Tambah `RECORD_AUDIO`, `VIBRATE` ke AndroidManifest.xml
- [ ] Fix guest login UUID
- [ ] Fix logika `isDummyKey`
- [ ] Setup TURN server
- [ ] Tambah React Error Boundary

### Fase 1 — Beta Tertutup (Minggu 3–6)
*100 pengguna terpilih dari komunitas motor Bandung*

- [ ] Onboarding flow yang jelas
- [ ] Push notification saat ada aktivitas di channel favorit
- [ ] Feedback in-app (thumbs up/down setelah sesi)
- [ ] Bug fix dari laporan beta testers
- [ ] Analytics (Posthog atau Mixpanel)

### Fase 2 — v1.0 Public (Bulan 2–3)

- [ ] Play Store listing (icon, screenshot, deskripsi)
- [ ] Sistem Premium (Midtrans payment integration)
- [ ] Channel database di Supabase (tidak lagi hardcoded)
- [ ] Push notification via FCM
- [ ] Pembaruan OTA certificate pinning
- [ ] iOS versi (Capacitor + Safari WebRTC)

### Fase 3 — Growth (Bulan 4–6)

- [ ] White-label dashboard untuk klien
- [ ] Channel moderasi tools (kick user, lock channel)
- [ ] Rekam pesan suara (voice memo)
- [ ] Integrasi peta (lokasi user di channel)
- [ ] SDK untuk integrasi pihak ketiga

### Fase 4 — Scale (Bulan 7–12)

- [ ] Self-hosted TURN server cluster (Indonesia region)
- [ ] End-to-end encryption per channel
- [ ] Transcripsi suara otomatis (Whisper API)
...
|---|---|---|---|
| WebRTC P2P gagal di jaringan seluler Indonesia | Tinggi | Tinggi | TURN server wajib, fallback base64 |
| Supabase downtime mempengaruhi semua koneksi | Sedang | Tinggi | Offline mode dengan localStorage, retry logic |
| Abuse channel (spam, konten tidak pantas) | Sedang | Sedang | Rate limiting, report system, moderasi |
| Competitor launch fitur serupa | Sedang | Sedang | Percepat white-label dan komunitas fitur unik |
| Biaya Supabase membengkak saat scale | Sedang | Sedang | Monitoring usage, implementasi rate limiting lebih awal |
| Play Store rejection | Rendah | Tinggi | Review guideline, tidak ada background audio tanpa notifikasi |
| Credential bocor terulang | Rendah | Sangat Tinggi | Secret scanning CI/CD, training developer, vault management |
| TURN server jadi bottleneck | Rendah | Tinggi | Load balancer, multiple TURN region, coturn cluster |

---

## 13. Dependensi & Asumsi

### Dependensi Eksternal

| Layanan | Fungsi | Tier | Estimasi Biaya |
|---|---|---|---|
| Supabase | Auth + Realtime + Database | Pro ($25/bulan) | Rp 400.000/bulan |
| Google Cloud (OAuth) | Login Google | Free tier | Gratis (hingga 100k req/hari) |
| TURN Server (Metered.ca / Coturn) | WebRTC relay | Berbayar | ~$30/bulan untuk 1TB bandwidth |
| Vercel (hosting web) | Deploy web app | Free/Pro | Rp 0–250.000/bulan |
| Google Play Store | Distribusi Android | Developer account | Rp 375.000 (sekali) |

### Asumsi

1. Supabase Realtime mampu menangani 1.000 concurrent connections per project di tier Pro
2. WebRTC P2P berhasil di ≥ 60% koneksi (sisanya pakai TURN)
3. Pengguna Indonesia memiliki rata-rata kecepatan upload ≥ 1 Mbps untuk discussion mode
4. Google OAuth tetap tersedia dan gratis untuk skala ini
5. Mayoritas pengguna awal menggunakan Android (> 80%)

---

## 14. Di Luar Cakupan (Out of Scope)

Fitur-fitur berikut **tidak** termasuk dalam scope v1.0:

- ❌ Chat teks — NextVWT adalah aplikasi suara, bukan messaging
- ❌ Panggilan video — audio-only sesuai konsep walkie-talkie
- ❌ Perekaman dan penyimpanan percakapan (kecuali voice memo di Fase 3)
- ❌ End-to-end encryption (dijadwalkan di Fase 4)
- ❌ Desktop app (Electron) — fokus mobile
- ❌ Koneksi antar-platform dengan Zello/Voxer
- ❌ Frekuensi radio fisik (internet-only)
- ❌ Dukungan multi-bahasa (hanya Bahasa Indonesia untuk v1.0)
- ❌ Channel berbasis geolokasi otomatis
...
| Istilah | Definisi |
|---|---|
| **PTT** | Push-to-Talk — mode komunikasi suara setengah-dupleks |
| **Channel** | Saluran komunikasi bernomor (0–999) yang bisa dimasuki pengguna |
| **Transmitter / TX** | Pengguna yang sedang mengirim suara |
| **Receiver / RX** | Pengguna yang sedang mendengarkan suara |
| **Callsign** | Kode identitas 5 karakter alfanumerik unik per pengguna |
| **VAD** | Voice Activity Detection — deteksi kehadiran suara manusia |
| **STUN** | Session Traversal Utilities for NAT — untuk menemukan IP publik |
| **TURN** | Traversal Using Relays around NAT — relay audio saat P2P gagal |
| **RLS** | Row Level Security — mekanisme akses data per-baris di PostgreSQL |
| **White-label** | Rebrand aplikasi untuk klien dengan identitas visual berbeda |
| **Half-duplex** | Tidak bisa kirim dan terima secara bersamaan (walkie-talkie konvensional) |
| **Full-duplex** | Bisa kirim dan terima secara bersamaan (seperti telepon biasa) |

### B. Referensi

- [Supabase Realtime Documentation](https://supabase.com/docs/guides/realtime)
- [WebRTC API MDN](https://developer.mozilla.org/en-US/docs/Web/API/WebRTC_API)
- [Capacitor Android Permissions](https://capacitorjs.com/docs/android/permissions)
- [Web Audio API](https://developer.mozilla.org/en-US/docs/Web/API/Web_Audio_API)
- [DSEG7 Font](https://www.keshikan.net/fonts-e.html)
